...
Trong console em thấy dòng thông báo Hello from JavaScript!
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
Điều này cho thấy khi trang web được tải, trình duyệt đã chạy file main.js và chạy lệnh console.log() để in thông báo ra console.
...
>
> - HTML: Xây dựng cấu trúc nội dung của trang web
...
>   - `number` là: kiểu dữ liệu dùng để biểu diễn số, bao gồm số nguyên và số thực.
>   - `string` là: kiểu dữ liệu dùng để biểu diễn chuỗi ký tự.
...
Dòng log hiển thị Xin chào, mình là Quân, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào? 
Trình duyệt báo lỗi Uncaught SyntaxError
...
Điều thú vị nhất em vừa khám phá được về console là console cho phép chạy thử code JavaScript ngay lập tức mà không cần sửa file.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)? 
Là lỗi quên dấu + khi nối chuỗi, xử lí bằng cách tự nhận thấy lỗi, tự sửa lại cú pháp và tải lại trang
...
Nếu điều kiện đúng thì sẽ thực hiện khối lệnh if, nếu không thì kiểm tra tiếp else if, nếu tất cả đều sai thì chạy else
...
`document.getElementById("status")` đang lấy phần tử HTML có id "status" trong trang web.
...
Thì Nội dung của đoạn <p id="status"> sẽ thay đổi thành: Xin chào! Đây là nội dung được thay đổi bằng JavaScript.
...
Ví dụ: document.body.style.fontSize = "20px";
...
Input xảy ra khi khi gõ hoặc thay đổi text còn click xảy ra khi nhấn chuột
...
>   DOM là mô hình biểu diễn trang HTML dưới dạng cây các đối tượng để JavaScript có thể truy cập và thay đổi nội dung trang web.
...
![img](images/chua_tuong_tac.png)
2. Sau khi nhấn “Chào”.
![img](images/nhan_chao.png)
3. Sau khi đổi nền sang màu đỏ.
![img](images/chuyen_mau_do.png)
4. Khi gõ tên và nhìn thấy lời chào xuất hiện.
![img](images/nhap_ten.png)
...
Điểm giống nhau: cả hai đoạn code đều gắn sự kiện click vào nút và hiển thị alert.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
document.getElementById có jQuery là $("#id") và addEventListener có jQuery là .on
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Giúp thao tác DOM dễ dàng hơn
>   2. Giúp xử lý sự kiện và hiệu ứng nhanh hơn
...
Em tò mò nhất về cách JavaScript thao tác với DOM để thay đổi nội dung trang web theo thời gian thực
